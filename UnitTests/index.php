...
$employee_idErr = $work_orderErr = $work_centerErr = $part_numberErr = $quantityErr = $start_timeErr = $end_timeErr = $sequence_numErr = $progressErr = $setup_timeErr = $die_setErr = $material_lotErr = $status_codeErr = $reporting_node = '';
...
// boolean to detect any errors and prevent sql query from running
$correct = True;

if ($_SERVER["REQUEST_METHOD"] == "POST") {
    //Employee ID
    $employee_id = test_input($_POST["employee_id"]);
    $employee_idErr = IdCheck($_POST["employee_id"]);
    //Work Order Number
    $work_order = test_input($_POST["work_order"]);
    $work_orderErr = WorkOrder($_POST["work_order"]);
    //Department
    ...
    $part_numberErr = PartNum($_POST["part_number"]);
    //Quantity
    $quantity = test_input($_POST["quantity"]);
    $quantityErr = Quant($_POST['quantity']);
    //Start Time
    $start_time = test_input($_POST["start_time"]);
    $start_timeErr = StartTime($_POST["start_time"]);
    //End Time
    $end_time = test_input($_POST["end_time"]);
    $end_timeErr = EndTime($_POST["start_time"], $_POST['end_time']);
    //Sequence
    $sequence_num = test_input($_POST["sequence_num"]);
    $sequence_numErr = Sequence($_POST["sequence_num"]);
    
    //Progress
    $progressErr = @Prog($_POST["progress"]);
    if($correct){
      $progress = test_input($_POST["progress"]);
    }
    //Setup Time
    $setup_time= test_input($_POST['setup_time']);
    $setup_timeErr = setupTime($_POST["setup_time"]);
    ...
    //retrieve the ip address and assign to variable
...
function IdCheck($IdInput){
  if (empty($IdInput)) {
      ...
      #$employee_id = test_input($IdInput);
...
function WorkOrder($OrderInput){
  if (empty($OrderInput)) {
        ...
        if (!preg_match("/^[a-zA-Z0-9' ]*$/",$OrderInput)) {
...
function EndTime($TimeStart, $TimeEnd){
  if (empty($TimeEnd)) {
...
function setupTime($setupInput){
  if ($setupInput != 0 && empty($setupInput)) {
      #$setup_timeErr = "Setup Time is required";
      $correct = False;
      return "Setup Time is required";
    } 
    else {
      if (!preg_match("#^[0-9]+$#",$setupInput)){
          $correct = False;
...
<form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
  ...
  <input type="text" id="employee_id" name="employee_id" value="<?php echo "$employee_name";?>">
...
if ($_SERVER["REQUEST_METHOD"] == "POST" and $correct) {
    ...
    $mysqli = new mysqli("ucr.proj.jmpdc23.com", "pdc1647_Test1114", "ucrpwdZG23", "pdc1647_Test1114");
      ...
      $sql="INSERT INTO pdc1647_Test1114.record (employee_id, work_order, reporting_node, work_center, part_number, quantity, start_time, end_time, sequence_num, progress, setup_time, die_set, material_lot, status_code, comments, department) VALUES ('$employee_id','$work_order','$reporting_node','$work_center','$part_number','$quantity','$start_time','$end_time','$sequence_num','$progress','$setup_time','$die_set','$material_lot','$status_code','$comments','$department')";
